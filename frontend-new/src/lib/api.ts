...
import { ApiError, handleApiResponse, makeRequest, qs } from './apiCore';
...
export const chatSessionsApi = {
  list: async (status?: ChatSessionStatus): Promise<BackendChatSession[]> => {
    const r = await makeRequest(`/api/chat/sessions${qs({ status })}`);
    return handleApiResponse<BackendChatSession[]>(r);
  },
  get: async (sessionId: string): Promise<BackendChatSession> => {
    const r = await makeRequest(`/api/chat/sessions/${encodeURIComponent(sessionId)}`);
    return handleApiResponse<BackendChatSession>(r);
  },
  ...
  getWorkspaces: async (sessionId: string): Promise<SessionWorkspacesResponse> => {
    const r = await makeRequest(
...
export const chatMessagesApi = {
  ...
  get: async (messageId: string): Promise<BackendChatMessage> => {
    const r = await makeRequest(`/api/chat/messages/${encodeURIComponent(messageId)}`);
    return handleApiResponse<BackendChatMessage>(r);
  },
  ...
  batchDelete: async (sessionId: string, messageIds: string[]): Promise<number> => {
    const r = await makeRequest(
...
export const chatAgentsApi = {
  ...
  get: async (agentId: string): Promise<BackendChatAgent> => {
    const r = await makeRequest(`/api/chat/agents/${encodeURIComponent(agentId)}`);
    return handleApiResponse<BackendChatAgent>(r);
  },
...
export const skillsApi = {
  ...
  get: async (skillId: string): Promise<BackendChatSkill> => {
    const r = await makeRequest(`/api/chat/skills/${encodeURIComponent(skillId)}`);
    return handleApiResponse<BackendChatSkill>(r);
  },
